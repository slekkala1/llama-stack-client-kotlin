...
private constructor(
    ...
    internal fun validity(): Int =
        accept(
            object : Visitor<Int> {
                override fun visitString(string: JsonValue) =
                    string.let { if (it == JsonValue.from(mapOf("type" to "string"))) 1 else 0 }

                override fun visitNumber(number: JsonValue) =
                    number.let { if (it == JsonValue.from(mapOf("type" to "number"))) 1 else 0 }

                override fun visitBoolean(boolean: JsonValue) =
                    boolean.let { if (it == JsonValue.from(mapOf("type" to "boolean"))) 1 else 0 }
                ...
                override fun visitUnion(union: JsonValue) =
                    union.let { if (it == JsonValue.from(mapOf("type" to "union"))) 1 else 0 }
            ...
            when (type) {
                "string" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(string = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "number" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(number = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "boolean" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(boolean = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "array" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(array = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "object" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                ...
                "json" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(json = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "union" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(union = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "chat_completion_input" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(chatCompletionInput = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "completion_input" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(completionInput = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
                "agent_turn_input" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { ParamType(agentTurnInput = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: ParamType(_json = json)
                }
            }
