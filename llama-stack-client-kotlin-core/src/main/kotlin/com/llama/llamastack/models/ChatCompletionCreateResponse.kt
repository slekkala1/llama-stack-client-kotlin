...
@JsonDeserialize(using = ChatCompletionCreateResponse.Deserializer::class)
@JsonSerialize(using = ChatCompletionCreateResponse.Serializer::class)
class ChatCompletionCreateResponse
private constructor(
    private val openaiChatCompletion: OpenAIChatCompletion? = null,
    private val chunk: ChatCompletionChunk? = null,
    private val _json: JsonValue? = null,
) {

    /** Response from an OpenAI-compatible chat completion request. */
    fun openaiChatCompletion(): OpenAIChatCompletion? = openaiChatCompletion

    /** Chunk from a streaming response to an OpenAI-compatible chat completion request. */
    fun chunk(): ChatCompletionChunk? = chunk

    fun isOpenAIChatCompletion(): Boolean = openaiChatCompletion != null

    fun isChunk(): Boolean = chunk != null

    /** Response from an OpenAI-compatible chat completion request. */
    fun asOpenAIChatCompletion(): OpenAIChatCompletion =
        openaiChatCompletion.getOrThrow("openaiChatCompletion")

    /** Chunk from a streaming response to an OpenAI-compatible chat completion request. */
    fun asChunk(): ChatCompletionChunk = chunk.getOrThrow("chunk")

    fun _json(): JsonValue? = _json

    fun <T> accept(visitor: Visitor<T>): T =
        when {
            openaiChatCompletion != null -> visitor.visitOpenAIChatCompletion(openaiChatCompletion)
            chunk != null -> visitor.visitChunk(chunk)
    ...
    fun validate(): ChatCompletionCreateResponse = apply {
        ...
        accept(
            object : Visitor<Unit> {
                override fun visitOpenAIChatCompletion(openaiChatCompletion: OpenAIChatCompletion) {
                    openaiChatCompletion.validate()
                }

                override fun visitChunk(chunk: ChatCompletionChunk) {
                    chunk.validate()
                }
            }
        )
    ...
    internal fun validity(): Int =
        accept(
            object : Visitor<Int> {
                override fun visitOpenAIChatCompletion(openaiChatCompletion: OpenAIChatCompletion) =
                    openaiChatCompletion.validity()

                override fun visitChunk(chunk: ChatCompletionChunk) = chunk.validity()
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is ChatCompletionCreateResponse && openaiChatCompletion == other.openaiChatCompletion && chunk == other.chunk /* spotless:on */
    }

    override fun hashCode(): Int = /* spotless:off */ Objects.hash(openaiChatCompletion, chunk) /* spotless:on */

    override fun toString(): String =
        when {
            openaiChatCompletion != null ->
                "ChatCompletionCreateResponse{openaiChatCompletion=$openaiChatCompletion}"
            chunk != null -> "ChatCompletionCreateResponse{chunk=$chunk}"
            _json != null -> "ChatCompletionCreateResponse{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid ChatCompletionCreateResponse")
        }

    companion object {

        /** Response from an OpenAI-compatible chat completion request. */
        fun ofOpenAIChatCompletion(openaiChatCompletion: OpenAIChatCompletion) =
            ChatCompletionCreateResponse(openaiChatCompletion = openaiChatCompletion)

        /** Chunk from a streaming response to an OpenAI-compatible chat completion request. */
        fun ofChunk(chunk: ChatCompletionChunk) = ChatCompletionCreateResponse(chunk = chunk)
    }

    /**
     * An interface that defines how to map each variant of [ChatCompletionCreateResponse] to a
     * value of type [T].
     */
    interface Visitor<out T> {

        /** Response from an OpenAI-compatible chat completion request. */
        fun visitOpenAIChatCompletion(openaiChatCompletion: OpenAIChatCompletion): T

        /** Chunk from a streaming response to an OpenAI-compatible chat completion request. */
        fun visitChunk(chunk: ChatCompletionChunk): T

        /**
         * Maps an unknown variant of [ChatCompletionCreateResponse] to a value of type [T].
         *
         * An instance of [ChatCompletionCreateResponse] can contain an unknown variant if it was
        ...
        fun unknown(json: JsonValue?): T {
            throw LlamaStackClientInvalidDataException(
                "Unknown ChatCompletionCreateResponse: $json"
            )
        }
    }

    internal class Deserializer :
        BaseDeserializer<ChatCompletionCreateResponse>(ChatCompletionCreateResponse::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): ChatCompletionCreateResponse {
            val json = JsonValue.fromJsonNode(node)

            val bestMatches =
                sequenceOf(
                        tryDeserialize(node, jacksonTypeRef<OpenAIChatCompletion>())?.let {
                            ChatCompletionCreateResponse(openaiChatCompletion = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<ChatCompletionChunk>())?.let {
                            ChatCompletionCreateResponse(chunk = it, _json = json)
                        },
                    )
                    .filterNotNull()
            ...
            return when (bestMatches.size) {
                ...
                0 -> ChatCompletionCreateResponse(_json = json)
    ...
    internal class Serializer :
        BaseSerializer<ChatCompletionCreateResponse>(ChatCompletionCreateResponse::class) {

        override fun serialize(
            value: ChatCompletionCreateResponse,
            generator: JsonGenerator,
            provider: SerializerProvider,
        ) {
            when {
                value.openaiChatCompletion != null ->
                    generator.writeObject(value.openaiChatCompletion)
                value.chunk != null -> generator.writeObject(value.chunk)
    ...
    private constructor(
        ...
        companion object {
            ...
            /**
             * Returns a mutable builder for constructing an instance of [OpenAIChatCompletion].
             *
        ...
        /** A builder for [OpenAIChatCompletion]. */
        class Builder internal constructor() {
            ...
            internal fun from(openaiChatCompletion: OpenAIChatCompletion) = apply {
                id = openaiChatCompletion.id
                choices = openaiChatCompletion.choices.map { it.toMutableList() }
                created = openaiChatCompletion.created
                model = openaiChatCompletion.model
                object_ = openaiChatCompletion.object_
                additionalProperties = openaiChatCompletion.additionalProperties.toMutableMap()
            }
            ...
            /**
             * Returns an immutable instance of [OpenAIChatCompletion].
             *
             ...
             */
            fun build(): OpenAIChatCompletion =
        ...
        fun validate(): OpenAIChatCompletion = apply {
            if (validated) {
                return@apply
            }
        ...
        private constructor(
             ...
             *
             * Unlike [logprobs], this method doesn't throw if the JSON field has an unexpected
             * type.
             */
            @JsonProperty("logprobs")
            @ExcludeMissing
            ...
            private constructor(
                ...
                companion object {
                    ...
                    /**
                     * A message representing the result of a tool invocation in an
                ...
                private constructor(
                    ...
                    private constructor(
                            ...
                            accept(
                                object : Visitor<Unit> {
                                    ...
                                    override fun visitOpenAIChatCompletionContentPartParams(
                                        ...
                                        openaiChatCompletionContentPartParams.forEach {
                                            it.validate()
                                        }
                                    }
                                }
                        ...
                        @JsonSerialize(
                            using = OpenAIChatCompletionContentPartParam.Serializer::class
                        )
                        ...
                        private constructor(
                            ...
                            interface Visitor<out T> {
                                ...
                                /**
                                 * Maps an unknown variant of [OpenAIChatCompletionContentPartParam]
                                 * to a value of type [T].
                                 *
                                 * An instance of [OpenAIChatCompletionContentPartParam] can contain
                                 * an unknown variant if it was deserialized from data that doesn't
                            ...
                            private constructor(
                                ...
                                private constructor(
                                    ...
                                    /**
                                     * @throws LlamaStackClientInvalidDataException if the JSON
                                     *   field has an unexpected type or is unexpectedly missing or
                                     *   null (e.g. if the server responded with an unexpected
                                     *   value).
                                     */
                                    ...
                                    /**
                                     * @throws LlamaStackClientInvalidDataException if the JSON
                                     *   field has an unexpected type (e.g. if the server responded
                ...
                private constructor(
                    ...
                    class Builder internal constructor() {
                        ...
                        /**
                         * The content of the "system prompt". If multiple system messages are
                         * provided, they are concatenated. The underlying Llama Stack code may also
                ...
                private constructor(
                    ...
                    private constructor(
                        ...
                        class Builder internal constructor() {
                             ...
                             * [Function] value instead. This method is primarily for setting the
                             * field to an undocumented or not yet supported value.
                             */
                        ...
                        private constructor(
                            ...
                            class Builder internal constructor() {
                                ...
                                internal fun from(function: Function) = apply {
                                    ...
                                    name = function.name
                                    additionalProperties =
                                        function.additionalProperties.toMutableMap()
                                }
            ...
            private constructor(
                 ...
                 *
                 * Unlike [refusal], this method doesn't throw if the JSON field has an unexpected
                 * type.
                 */
                @JsonProperty("refusal")
                @ExcludeMissing
                ...
                class Builder internal constructor() {
                     ...
                     *
                     * You should usually call [Builder.refusal] with a well-typed `List<Refusal>`
                     * value instead. This method is primarily for setting the field to an
                     * undocumented or not yet supported value.
                     */
                ...
                private constructor(
                    ...
                    class Builder internal constructor() {
                         ...
                         * `List<TopLogprob>` value instead. This method is primarily for setting
                         * the field to an undocumented or not yet supported value.
                         */
                    ...
                    private constructor(
                         ...
                         * Returns the raw JSON value of [token].
                         *
                         * Unlike [token], this method doesn't throw if the JSON field has an
                         * unexpected type.
                         */
                        @JsonProperty("token")
                        @ExcludeMissing
                         ...
                         * Returns the raw JSON value of [bytes].
                         *
                         * Unlike [bytes], this method doesn't throw if the JSON field has an
                         * unexpected type.
                         */
                        @JsonProperty("bytes")
                        @ExcludeMissing
                        ...
                        class Builder internal constructor() {
                             ...
                             * Sets [Builder.token] to an arbitrary JSON value.
                             *
                             * You should usually call [Builder.token] with a well-typed [String]
                             * value instead. This method is primarily for setting the field to an
                             * undocumented or not yet supported value.
                             */
                            fun token(token: JsonField<String>) = apply { this.token = token }
                             ...
                             * `List<Long>` value instead. This method is primarily for setting the
                             * field to an undocumented or not yet supported value.
                             */
                            fun bytes(bytes: JsonField<List<Long>>) = apply {
                                this.bytes = bytes.map { it.toMutableList() }
        ...
        override fun equals(other: Any?): Boolean {
            ...
            return /* spotless:off */ other is OpenAIChatCompletion && id == other.id && choices == other.choices && created == other.created && model == other.model && object_ == other.object_ && additionalProperties == other.additionalProperties /* spotless:on */
        }
        ...
        override fun toString() =
            "OpenAIChatCompletion{id=$id, choices=$choices, created=$created, model=$model, object_=$object_, additionalProperties=$additionalProperties}"
    }
}
